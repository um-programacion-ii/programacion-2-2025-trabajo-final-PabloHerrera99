...
import com.evento.backend.service.IntegranteService;
...
class IntegranteResourceIT {
    ...
    @Mock
    private IntegranteRepository integranteRepositoryMock;

    @Autowired
    private IntegranteMapper integranteMapper;

    @Mock
    private IntegranteService integranteServiceMock;
    ...
    public static Integrante createEntity(EntityManager em) {
        Integrante integrante = new Integrante().nombre(DEFAULT_NOMBRE).apellido(DEFAULT_APELLIDO).identificacion(DEFAULT_IDENTIFICACION);
        // Add required entity
        Evento evento;
    ...
    public static Integrante createUpdatedEntity(EntityManager em) {
        Integrante updatedIntegrante = new Integrante()
        ...
        updatedIntegrante.setEvento(evento);
        return updatedIntegrante;
    }
    ...
    void getAllIntegrantesWithEagerRelationshipsIsEnabled() throws Exception {
        when(integranteServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restIntegranteMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(integranteServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllIntegrantesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(integranteServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restIntegranteMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(integranteRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    void partialUpdateIntegranteWithPatch() throws Exception {
        ...
        partialUpdatedIntegrante.nombre(UPDATED_NOMBRE).identificacion(UPDATED_IDENTIFICACION);
